...
enum Algo {
    ...
    A_HFJ85, A_HFJ123, A_HFJ128, A_HFJAUTO,
...
// ---------------------------------------------------------------------------
static void fjcounts() {
    // known MergeInsertion worst-case counts F(n), n = 0..22; beyond that we
    // report measured avg/max plus the binary-insertion-from-scratch average
    // on the same inputs (bin_avg) — the per-block FJ gain is bin_avg - fj_avg.
    const int F[] = {0, 0, 1, 3, 5, 7, 10, 13, 16, 19, 22, 26, 30, 34, 38,
                     42, 46, 50, 54, 58, 62, 66, 71};
    std::printf("n, fj_avg, fj_max, bin_avg, fj_gap_vs_lb, F(n), lg_nfact\n");
    std::mt19937_64 rng(99);
    for (int n = 1; n <= 128; ++n) {
        double sum = 0, bsum = 0;
        u64 mx = 0;
        int trials = n <= 64 ? 20000 : 8000;
        std::vector<u64> v(n);
        ...
        for (int t = 0; t < trials; ++t) {
            ...
            if (!std::is_sorted(w.begin(), w.end())) {
                std::printf("BININS BUG at n=%d\n", n);
                return;
            }
        }
        double lb = std::lgamma(n + 1.0) / std::log(2.0);
        char fs[8];
        if (n <= 22) std::snprintf(fs, sizeof fs, "%3d", F[n]);
        else std::snprintf(fs, sizeof fs, "  -");
        std::printf("%2d, %8.3f, %3" PRIu64 ", %8.3f, %6.3f, %s, %8.3f\n",
                    n, sum / trials, mx, bsum / trials, sum / trials - lb, fs,
                    lb);
    }
}
...
// bench
// ---------------------------------------------------------------------------
static void bench(const std::string& algo, const std::string& dist, size_t n,
                  u64 seed, int reps, const std::string& mode) {
    const AlgoInfo* ai = find_algo(algo);
    std::vector<u64> master = gen_dist(dist, n, seed);
    u64 mastersum = 0;
    for (u64 x : master) mastersum += x;
    std::vector<u64> work(n);

    auto verify = [&]() -> int {
        if (!std::is_sorted(work.begin(), work.end())) return 0;
        u64 s = 0;
        for (u64 x : work) s += x;
        return s == mastersum ? 1 : 0;
    };
    ...
    if (mode == "time") {
        for (int r = 0; r < reps; ++r) {
            ...
            run_algo(ai->id, work.data(), n, lab::RawLess{});
            u64 t1 = now_ns();
            std::printf("%s,%s,%zu,%" PRIu64 ",time,%d,%" PRIu64 ",0,0,0,%d\n",
                        algo.c_str(), dist.c_str(), n, seed, r, t1 - t0, verify());
        }
    } else if (mode == "count") {
        std::memcpy(work.data(), master.data(), n * sizeof(u64));
        lab::g_comps = 0;
...
int main(int argc, char** argv) {
    ...
    if (cmd == "fjcounts") { fjcounts(); return 0; }
    if (cmd == "list") {
        for (auto& ai : ALGOS)
    ...
    if (cmd == "bench" && argc >= 8 && argc <= 10) {
        ...
        bench(argv[2], argv[3], std::strtoull(argv[4], nullptr, 10),
              std::strtoull(argv[5], nullptr, 10), std::atoi(argv[6]), argv[7]);
        return 0;
    }
